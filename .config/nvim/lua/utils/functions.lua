...
X.toggled_terminals = {nil, false, false, false }
X.toggle_term_with_count = function(count)
		X.toggled_terminals[count] = not X.toggled_terminals[count]
		vim.api.nvim_command(string.format('exe %d "ToggleTerm"', count or 1))

    -- Set up an autocmd to trigger after the terminal is entered
end

--.latest_esc_press = 0
--.handle_terminal_esc = function()
--   local current_time = vim.fn.reltime()
--	local delta = vim.fn.reltimefloat(current_time) - vim.fn.reltimefloat(X.latest_esc_press)
--   -- Check the time difference since the last <Esc> press
--   if delta > 0 and delta < 1.0 then
--       -- If pressed twice quickly, toggle off the terminal
--		--	vim.fn.feedkeys([[<C-\><C-n><CR>]], 'n')
--			vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes('<C-\\><C-n>', true, true, true), 'n', true)


--	--	vim.fn.feedkeys([[ToggleTerm<cr>]],'n')
--       vim.cmd([[ToggleTerm]])
--   else
--       -- Update the last press 18:51
--       X.latest_esc_press = vim.fn.reltime()
--   end
--end
